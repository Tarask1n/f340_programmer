...
// taraskin Note: !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//#define STRING_NAME_LEN sizeof("EP01 USB Programmer")
#define STRING_NAME_LEN sizeof("Taraskin's USB Programmer")
static SEGMENT_VARIABLE(STRING_NAME[STRING_NAME_LEN], const U8, SEG_CODE) = "Taraskin's USB Programmer";

#define STRING_VERSION_LEN sizeof("1.0.0.3")
static SEGMENT_VARIABLE(STRING_VERSION[STRING_VERSION_LEN], const U8, SEG_CODE) = "1.0.0.3";
...
static SEGMENT_VARIABLE(ARRAY_WRITE_OK[3], const U8, SEG_CODE) = {0x41, 0x4f, 0x4b}; // 'AOK'
static SEGMENT_VARIABLE(ARRAY_WRITE_ER[3], const U8, SEG_CODE) = {0x41, 0x45, 0x52}; // 'AER'
//-----------------------------------------------------------------------------
// Handle_Vendor
//-----------------------------------------------------------------------------
//
...
// Additional bRequest states
      ...
      case GET_WRITE_STATE:
         ...
         break;
      case SELF_RST_BL:
        if  (Setup.wValue.U16 == 0x424C)   // 0x424C ='BL'
        {
            Self_Restart_Bootloader (); 
        }
        break;
...
//
void Self_Restart_Bootloader (void)
{
    // Write to Flash 'LB' sign to load into bootloader
    ...
    // Write two bytes to XDATA the sign to force start bootloader
    *pBoot_By_FW_Request = 0x424C;  // 'LB'
    // Wait to allow the control transfer
    // to finish before resetting the device
//    Delay ();
    Delay_us (80);
...
void Get_Write_State (void)
{
    if (StatusSPIWriteOk)
    {
        // Return OK write state
        DataPtr = (U8*)&ARRAY_WRITE_OK;
        DataSize = sizeof(ARRAY_WRITE_OK);
    }
    else
    {
        // Return ER write state
        DataPtr = (U8*)&ARRAY_WRITE_ER;
        DataSize = sizeof(ARRAY_WRITE_ER);
    }
